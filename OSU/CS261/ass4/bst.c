...
{
    assert(tree != 0); /*see if tree is null*/
    tree->cnt  = 0;
    tree->root = 0;

    //printf("Tree has been initialized\n");
}
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    assert(tree != 0);
    assert(val != 0);
    /*Loop through tree until end. See while loop for delimiter*/
	...
	if (compare(val, temp->val) == -1)
	    temp = temp->left; //if compare comes back with -1 then go left
	else
	    temp = temp->right; //if it is not -1, (1) then go right!
    }
...
{
    assert(cur != 0);
    //printf("starting function _leftMost\n");
    while (cur->left !=0){
	cur = cur->left;
    }
    return cur->val;
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
    ...
    if(compare(val, cur->val) == 0) {  /* if current value is one to be removed! */
	if(cur->right == 0 ) {
	    temp = cur->left;
	    free(cur);
	    return temp;
